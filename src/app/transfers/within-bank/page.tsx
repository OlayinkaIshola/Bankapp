...
export default function WithinBankTransferPage() {
  const dispatch = useAppDispatch();
  const { accounts } = useAppSelector((s) => s.accounts);
  const { isLoading } = useAppSelector((s) => s.transactions);
  const { register, handleSubmit, reset } = useForm<{ fromAccountId: string; toAccountNumber: string; amount: number; description: string }>();

  useEffect(() => { dispatch(fetchAccounts()); }, [dispatch]);

  const onSubmit = async (data: { fromAccountId: string; toAccountNumber: string; amount: number; description: string }) => {
    await dispatch(createTransfer({ ...data, transferType: 'external' } as any));
    alert('Within-bank transfer submitted');
    reset();
  };
  ...
  return (
    <RequireAuth>
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        <h1 className="text-2xl font-bold text-gray-900">Within Bank Transfers</h1>
