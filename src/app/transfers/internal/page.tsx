...
export default function InternalTransferPage() {
  ...
  const { register, handleSubmit, reset } = useForm<{ fromAccountId: string; toAccountId: string; amount: number; description: string }>();
  ...
  const onSubmit = async (data: { fromAccountId: string; toAccountId: string; amount: number; description: string }) => {
    await dispatch(createTransfer({ ...data, toAccountNumber: '', transferType: 'internal' } as any));
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-4">
          ...
          <div>
            ...
            <select {...register('toAccountId', { required: true })} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
              <option value="">Select account</option>
              {accounts.map((a) => (<option key={a.id} value={a.id}>{a.accountType} ({a.accountNumber})</option>))}
            </select>
          </div>
